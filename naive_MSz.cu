    ...
    for (size_t i = 0; i < recvbufs.size(); ++i) {
        if (recvbufs[i] != nullptr) {
            ...
            dim3 gridSize_unpack((send_count + blockSize.x - 1) / blockSize.x);
                                                               ...
                                                               // local_x, local_y, local_z, dx, dy, dz, rank);
            // unpack_sparse_edits_kernel<T><<<gridSize_unpack, blockSize>>>(data,
            //     delta_counter,
            //     edits,
            //     recvbufs[i],  // received sparse gIdx
            //     send_count,
            //     padded_x, padded_y,
...
// Sparse ghost layer exchange using CUDA-aware MPI
// This function assumes HALO = 1, and sends only the ghost + outermost core layer cells that are modified (edits[i] == -1)
...
void getdata(const std::string &filename, T *&input_data_host, size_t data_size) {
    ...
    // cudaMemcpy(input_data_host, h_buffer.data(), data_size * sizeof(uint16_t), cudaMemcpyHostToDevice);    
...
void save_time_records(const std::vector<std::map<std::string, double>>&time_records,
    ...
    //     std::string filename = filename_prefix + std::to_string(i) + ".txt";
...
__global__ void extract_false_path(T *decp_data, T *input_data, 
    ...
    if(DS_M[i] != dec_DS_M[i]){
        ...
        if (start_vertex!=-1){
            ...
            // if(rank == 0 && true_index == 38070308) printf("%d %d %.17f %.17f %.17f %.17f\n ", false_index, true_index, decp_data[false_index], input_data[false_index] - bound, decp_data[true_index], input_data[true_index] - bound);
...
void c_loops(size_t data_size, T *input_data_host, T *decp_data_host, size_t padded_x, 
    ...
    // std::cout<<"path compression begin"<<std::endl;
    PathCompression<<<gridSize, blockSize>>>(DS_M, data_size, padded_x, padded_y, padded_z, 0);
    cudaDeviceSynchronize();
    PathCompression<<<gridSize, blockSize>>>(AS_M, data_size, padded_x, padded_y, padded_z, 0);
    cudaDeviceSynchronize();
    // std::cout<<"path compression ended"<<std::endl;
    ...
    size_t shmem = ((TX*TY*TZ + 31)/32) * sizeof(uint32_t) + QCAP*sizeof(int);
    ...
    while(global_count_f_max > 0 || global_count_f_min > 0 || global_count_f_dir > 0){
        ...
        // if(rank == 0 ) std::cout<<host_count_f_max << ", " << host_count_f_min << ", " << host_count_f_dir<<std::endl;
        std::vector<float> c_temp;
        
        ite++;
        
        
        while(host_count_f_max > 0 || host_count_f_min > 0){
            ...
            iscriticle<T><<<gridSize, blockSize, shmem>>>(dec_DS_M, dec_AS_M, de_direction_as, de_direction_ds, all_min, input_data_host, decp_data_host, edits, delta_counter, 
        ...
        // start_time = MPI_Wtime();
        c_loops+= end_time - start_time - prev;
        prev = end_time - start_time;
        // }   
        
        err = cudaMemcpyToSymbol(count_f_dir, &initialValue, sizeof(unsigned int));

        PathCompression<<<gridSize, blockSize>>>(dec_DS_M, data_size, padded_x, padded_y, padded_z, 0);
        cudaDeviceSynchronize();
        PathCompression<<<gridSize, blockSize>>>(dec_AS_M, data_size, padded_x, padded_y, padded_z, 0);
        cudaDeviceSynchronize();

        init_edits<<<gridSize, blockSize>>>(data_size, edits);
        err = cudaMemcpy(d_send_flag, &zero, sizeof(int), cudaMemcpyHostToDevice);
        extract_false_path<T><<<gridSize, blockSize>>>(decp_data_host, input_data_host, 
            DS_M, dec_DS_M, AS_M, dec_AS_M, 
            data_size, padded_x, padded_y, padded_z, or_types,
            de_direction_ds, de_direction_as,
            edits, delta_counter, q, bound, 
            delta, rank, ite,  
            d_send_flag);
        ...
        temp.push_back(end_time - start_time);
        // start_time = MPI_Wtime();
        r_loops+=end_time - start_time - prev;
        prev = end_time - start_time;
            // start_time = MPI_Wtime();
        ...
        float global_c;
        MPI_Reduce(&c_loop, &global_c, 1, MPI_FLOAT, MPI_MAX, 0, MPI_COMM_WORLD);
        // MPI_Barrier(cart_comm);

        float global_r;
        MPI_Reduce(&r_loop, &global_r, 1, MPI_FLOAT, MPI_MAX, 0, MPI_COMM_WORLD);
        ...
        end_time = MPI_Wtime();
        datatransfer += end_time - start_time;
        comm_time.push_back(end_time - start_time);
        
        temp.push_back(end_time - start_time);
        // start_time = MPI_Wtime();
        datacoo+=end_time - start_time - prev;
        prev = end_time - start_time;
        ...
        // if (rank == 0) {
            ...
            datatransfer += end_time - start_time;
            comm_time.push_back(end_time - start_time);
            times.push_back(std::map<std::string, double>{{"comm_time", end_time - start_time}});
            temp.push_back(end_time - start_time);
            // start_time = MPI_Wtime();
            datacoo+=end_time - start_time - prev;
            prev = end_time - start_time;
...
void gather_cost_sparse(std::string file_path, std::string filename, T* decp_data, T* decp_data_host, T* decp_data_copy_host, T* decp_data_copy,
    ...
    for (int i = 1; i < world_size; ++i) {
        ...
        // off_edit_pos[i]  = off_edit_pos[i-1]  + all_sz_edit_pos[i-1];
...
int main(int argc, char** argv) {
    ...
    cudaError_t err = cudaMalloc(&DS_M, data_size * sizeof(int));
    err = cudaMalloc(&dec_DS_M, data_size * sizeof(int));
    err = cudaMalloc(&AS_M, data_size * sizeof(int));
    err = cudaMalloc(&dec_AS_M, data_size * sizeof(int));
    err = cudaMalloc(&de_direction_as, data_size * sizeof(int));
    ...
    std::ofstream outFilep("Global_naive_MSz_performance_with_ite_cuda_"+compressor_id+"_"+filename+"_rank_"+std::to_string(rank)+".txt", std::ios::app);
    ...
    if(rank == 0){
        ...
        std::ofstream outFile3("./stat_result/Global_naive_base_result_"+filename+"_"+compressor_id+"_Maxonly_final"+std::to_string(size)+".txt", std::ios::app);
