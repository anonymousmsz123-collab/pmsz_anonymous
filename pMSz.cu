...
__global__ void iscriticle(int *DS_M, int *AS_M, int *de_direction_as, int *de_direction_ds, 
                            ...
                            T uTh, T lTh, int data_type=0, int preserveMSS = 0, int checkFinal = 0, int maxOnly = 0){
    ...
    if (data_type == 0) {
        ...
        if (((is_minima && original_type!= -2) || (!is_minima && original_type == -2) || (is_maxima &&  original_type!= -1) || (!is_maxima && original_type == -1)) && maxOnly == 0){
                count_f_dir+=1;
                return;
        }
        ...
        if(smallest_index != original_smallest_index && maxOnly == 0){
            count_f_dir+=1;
    ...
    int dx, int dy, int dz, int rank)
{
    ...
    // double recvValue = recvbuf[2 * idx];
    // T recvEdit = recvbuf[2 * idx + 1];
    double currentV = data[global_idx];
    // T currentEdit = edits[global_idx];
    // T recvEdit = recvbuf[idx];
    T currentDelta = delta_counter[global_idx];
    T recvDelta = recvbuf[idx];
    // if(currentV > recvValue && currentV != INVALID_VAL && recvValue != INVALID_VAL){
    //     delta_counter[global_idx] += 1;
    //     data[global_idx] = recvValue;
    // }
    // if(currentV!=INVALID_VAL && currentEdit != 0 && recvEdit == 1){
    //     delta_counter[global_idx] += 1;
    //     data[global_idx] -= delta;
    // }
...
void c_loops(size_t data_size, T *input_data_host, T *decp_data_host, size_t padded_x, 
            ...
            int offset_z, int size, T uTh, T lTh, int iteStep, int maxOnly){
    ...
    float comm = 0, comp = 0;
    float prev = 0;
    while(global_count_f_max > 0 || global_count_f_min > 0 || (global_count_f_dir > 0 && maxOnly == 0)){
        ...
        while(host_count_f_max > 0 || host_count_f_min > 0  || (host_count_f_dir > 0  && maxOnly == 0)){
        ...
        if (rank == 0) {
            ...
            temp.push_back((end_time - start_time) - prev);
            prev = end_time - start_time;
...
int main(int argc, char** argv) {
    ...
    c_loops<double>(data_size, device_input, device_decp, padded_x, 
        ...
        sendbuff_right, sendbuff_left, sendbuff_top, sendbuff_bottom, offset_x, offset_y, offset_z, size, uTh, lTh, iteStep, maxOnly
    );
    
    MPI_Barrier(cart_comm);
    int c1 = 0;  
    std::ofstream outFilep("Global_performance_with_ite_cuda_"+compressor_id+"_"+filename+"_rank_"+std::to_string(rank)+"_"+std::to_string(maxOnly)+".txt", std::ios::app);
    ...
    if(rank == 0){
        ...
        std::ofstream outFile3("./stat_result/Global_base_result_"+filename+"_"+compressor_id+"_Maxonly_final"+std::to_string(size)+"_"+std::to_string(maxOnly)+".txt", std::ios::app);
